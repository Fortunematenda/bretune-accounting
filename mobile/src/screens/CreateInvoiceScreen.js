...
export default function CreateInvoiceScreen() {
  ...
  const customersQuery = useApiQuery(['invoiceCustomers'], () => api.listCustomers({ limit: 100 }));
  const productsQuery = useApiQuery(['invoiceProducts'], () => api.listProducts({ limit: 100, isActive: true }));
  const nextNumberQuery = useApiQuery(['nextInvoiceNumber'], () => api.getNextInvoiceNumber());
  const [form, setForm] = useState({ clientId: route.params?.customerId || '', issueDate: '', dueDate: '', notes: '', productId: '', description: '', quantity: '1', unitPrice: '', taxRate: '0.15', send: false });
  ...
  const invoiceNumber = nextNumberQuery.data?.invoiceNumber || nextNumberQuery.data || 'Loading...';
  ...
  async function createNewItem() {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['invoiceProducts'] });
  ...
  async function submit() {
    ...
    try {
      ...
      await add({ type: 'Create invoice', summary: `Invoice for ${selectedCustomer?.companyName || selectedCustomer?.contactName || form.clientId}` });
  ...
  return (
    <Screen scroll>
      <Card title="Create invoice" subtitle={`Invoice number: ${invoiceNumber}`}>
        ...
        {!selectedCustomer ? <Text style={{ color: '#dc2626', marginTop: 6 }}>Open this form from a customer to create the invoice automatically for that customer.</Text> : null}
        <Field label="Issue Date" value={form.issueDate} onChangeText={(value) => setForm((prev) => ({ ...prev, issueDate: value }))} placeholder="YYYY-MM-DD" />
        <Field label="Due Date" value={form.dueDate} onChangeText={(value) => setForm((prev) => ({ ...prev, dueDate: value }))} placeholder="YYYY-MM-DD" />
        ...
        <AppButton title={saving ? 'Creating...' : 'Create invoice'} onPress={submit} loading={saving} disabled={!form.clientId || !form.description || !form.unitPrice} />
      </Card>
